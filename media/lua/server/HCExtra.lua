...
for k in pairs(farming_vegetableconf.props) do
	if (farming_vegetableconf.props[k]["seedCollect"] > 0) then 
    for i=1,allScriptItems:size() do
        local scriptItem = allScriptItems:get(i-1);
   
        --if scriptItem:getName() == farming_vegetableconf.props[k]["id"] then
        local vegetableName = string.gsub(farming_vegetableconf.props[k]["vegetableName"], "Base.", ""); 
...
if (itemName ~= "HCHerbtable2" and itemName ~= "KitchenKnife") then
	...
	for k in pairs(farming_vegetableconf.props) do
    	...
    	if (itemName == vegetableName) then
    		print (vegetableName)
			player:sendObjectChange('addItemOfType', { type = farming_vegetableconf.props[k]["seedName"], count = farming_vegetableconf.props[k]["seedCollect"] })
    	end
	end



	-- local vegetableName = farming_vegetableconf.props[itemName]["seedName"]
	--vegetableName = string.gsub(vegetableName, "Base.", "")
    --vegetableName = string.gsub(vegetableName, "farming.", "")
    --vegetableName = string.gsub(vegetableName, "Hydrocraft.", "")
    
	--player:sendObjectChange('addItemOfType', { type = vegetableName, count = farming_vegetableconf.props[itemName]["seedCollect"] })
end
end 
end
